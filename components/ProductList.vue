<template>
    <div class="container">
        <div class="products-grid">
            <div v-if="data" v-for="auto in data.results">
                <img :src="auto.images[0].file.url" alt="">
                <h4>{{ auto.name }}</h4>
                <p>{{ auto.price }}$</p>
                <NuxtLink :to="`/posts/${auto.slug}`">Anschauen</NuxtLink>
            </div>
...
export default {
    ...
    async created() {
        ...
        this.data = await swell.products.list({
            limit: 25,
            page: 1
        }).then(res => { return res })
    },
